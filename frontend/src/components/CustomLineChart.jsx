  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      const groupedItemsForTooltip = dataPoint.items.reduce((acc, item) => {
        ...
        acc[categoryName].totalAmount += Number(amount);
        return acc;
      }, {});
      ...
      return (
        <div className="bg-white shadow-md rounded-lg p-2 border border-gray-300">
          ...
          {dataPoint.userName && (
            <p className="text-xs text-blue-600 font-medium mb-1">👤 {dataPoint.userName}</p>
          )}
    ...
    <div className="bg-white">
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={data}>
          ...
          <Area type="monotone" dataKey="totalAmount" stroke="#875cf5"
            fill="url(#expenseGradient)" strokeWidth={3} dot={{ r: 3, fill: "#ab8df8" }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
